...
class PlayerViewModel @JvmOverloads constructor(
    ...
    private val downloadAheadAmount = animeDownloadPreferences.autoDownloadWhileWatching().get()
    ...
    private fun filterEpisodeList(episodes: List<Episode>): List<Episode> {
        ...
        // Each `&&` pair is parenthesised: && already binds tighter than ||, so this changes
        // nothing except that the reader no longer has to remember that.
        val episodesForPlayer = episodes.filterNot {
            ...
            (anime.unseenFilterRaw == Anime.EPISODE_SHOW_SEEN && !it.seen) ||
                ...
                (anime.downloadedFilterRaw == Anime.EPISODE_SHOW_DOWNLOADED && !downloaded) ||
                (anime.downloadedFilterRaw == Anime.EPISODE_SHOW_NOT_DOWNLOADED && downloaded) ||
