 ...
 *
 * Aniyomi added both to Mihon's own class. One is plainly anime-only — Mihon has no airing times to
 * show. The other is not, but Mihon still does not offer it, and adding it to Mihon's file is the
 * move this project exists to avoid. Preference keys are Aniyomi's, so an imported install keeps
 * both settings.
 */
