...
/*
 * Adopted from Mihon's own Shizuku installer rather than carried over from Aniyomi.
 *
 * Aniyomi's version drove Shizuku through `Shizuku.newProcess`, which the SDK version Mihon pins
 * has made private — so it could not have been kept as it was. Mihon had already moved to binding
 * an AIDL user service, which is the supported approach, so this takes theirs wholesale and
 * changes only the class name and the base it extends.
 *
 * It binds the very `mihon.app.shizuku.ShellInterface` Mihon declares, with the same service tag,
 * so the two installers share one Shizuku user service instead of standing up a second.
 */
