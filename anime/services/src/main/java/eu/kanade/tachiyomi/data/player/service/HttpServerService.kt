...
class HttpServerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val builder = notificationBuilder(AnimeNotifications.CHANNEL_HTTP_SERVER) {
            setLargeIcon(BitmapFactory.decodeResource(resources, R.mipmap.ic_launcher))
            setSmallIcon(AnimeR.drawable.ic_dns_24dp)
            addAction(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            startForeground(
                AnimeNotifications.ID_HTTP_SERVER,
                builder.build(),
                ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC,
            )
        } else {
            startForeground(AnimeNotifications.ID_HTTP_SERVER, builder.build())
        }
    ...
    companion object {
        ...
        /**
         * Starts the server and waits for it to report itself ready.
         *
         * Aniyomi had this on `MainActivity`, which the downloader then had to reach up into to
         * fetch a video. Nothing in it touches an activity — it starts a service and waits on a
         * flow — so it lives on the service it starts, and the layer below can call it.
         *
         * @return whether the server came up, and the port it is listening on.
         */
        suspend fun start(
            context: Context,
            sourceId: Long,
            ...
            context.startService(
                Intent(context, HttpServerService::class.java).putExtra(EXTRA_SOURCE_ID, sourceId),
            )

            val ready = withTimeoutOrNull(timeout) { isRunning.first { it } }
            return Pair(ready == true, port)
        }
    }
}
